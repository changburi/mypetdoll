            ...
            <tr v-for="cloth in clothes" :key="cloth['id']">
                    ...
                    <nuxt-link :to='{path: "/cloth/" + cloth["id"]}'>
                        ...
                        <tr align="center" valign="middle">
                            <td>{{cloth['id']}}</td>
                            <td><img width="50" height="50" :src='"http://mypetdoll.ga/uploads/" + cloth["img" ]'></td>
...
export default {
    ...
    async asyncData () {
        let data = await axios.get('http://mypetdoll.ga/api/v1.0/admin/clothes')
    ...
    methods:{
        async getPage (page) {
            let url = `http://mypetdoll.ga/api/v1.0/admin/clothes?page=${page}`
        ...
        async registry (cloth) {
            ...
            let fileDom = document.querySelector('#img')
                if(this.name && this.price && this.category && fileDom.files[0]) {
                    ...
                    let data = await axios.post('http://mypetdoll.ga/api/v1.0/admin/clothes/registry', formData, {
                        headers: {
...
//                this.$router.replace({ path: '/unregistry', force:true })                    
        ...
        async cancel(cloth) {
            let url = 'http://mypetdoll.ga/api/v1.0/admin/clothes/registry'
            let clothId= cloth['id']
            let name = cloth['name']
