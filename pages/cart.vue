<template>
    <div class="container">
        <table>
            <tr><th>번호</th><th>사진</th><th>품목</th><th>제품명</th><th>단가</th><th>수량</th><th>합계</th><th>취소</th><th>누적</th><th>제품 상세</th></tr>            
            <tr v-for="cart in carts" :key="cart['id']">
                <td>{{cart['id']}}</td>
                <td><img width="50" height="50" :src='"http://mypetdoll.ga/uploads/" + cart["clothe.img"]'></td>
                <td>{{cart['clothe.name']}}</td>                
                <td>{{cart['clothe.name']}}</td>
                <td>{{cart['clothe.price']}}</td>
                <td>{{cart['count']}}</td>
                <td>{{cart['clothe.price'] * cart['count']}}</td>
...
export default {
    ...
    async asyncData () {
        let data = await axios.get('http://mypetdoll.ga/api/v1.0/cart')
//        alert('asyncData')
        return {
    ...
    methods : {
        async getPage (page) {
            let url = `http://mypetdoll.ga/api/v1.0/cart?page=${page}`
        ...
        async signout () {
//            alert('로그 아웃하셨습니다.')
            let url = 'http://mypetdoll.ga/api/v1.0/users/signout'
        ...
        async cancel (cart) {
            let url = 'http://mypetdoll.ga/api/v1.0/cart'
...
//                reloadPage()
        ...
        async buy (carts) {
            let url = 'http://mypetdoll.ga/api/v1.0/purchase'
