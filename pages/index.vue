                        ...
                        <tr v-for="cloth in clothes" :key="cloth['id']">
                            <nuxt-link :to='{path: "/cloth/" + cloth["id"]}'>
                                ...
                                <tr align="center" valign="middle">                    
                                    <td><img width="200" height="200" :src='"http://mypetdoll.ga/uploads/" + cloth["img" ]'></td>
...
function getPagination ({currentPage, totalCount, limit}) {
    ...
    export default {
        ...
        async asyncData () {
            let data = await axios.get('http://mypetdoll.ga/api/v1.0/clothes')
            .catch((error) => {
                if(error.response) {
    ...
    methods : {
        ...
        async getPage (page) {
            let url = `http://mypetdoll.ga/api/v1.0/clothes?page=${page}&category=${this.category}`
