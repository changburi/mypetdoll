<template>
    <div class="container">
        ...
        <table>
            <tr><th>사용자</th><th>품목</th><th>품명</th><th>단가</th><th>수량</th><th>총액 (단가x수량)</th><th>주문날짜</th></tr>
            <tr  v-for="purchase in purchases" :key="purchase['id']">
                <td>{{purchase['user.uid']}}</td>
                <td>{{purchase['clothe.category']}}</td>                
...
export default {
    async asyncData () {
        let data = await axios.get('http://mypetdoll.ga/api/v1.0/purchase')
        return {
    ...
    methods : {
        async getPage (page) {
            let url = `http://mypetdoll.ga/api/v1.0/purchase?page=${page}`
